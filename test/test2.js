...
(async function () {
  ...
  for (let index = 0; index < avatarsNodes.length; index++) {
    ...
    const childrenNodes = avatarsNodes[index].querySelectorAll('a[href^="/name/"]');
    if (!childrenNodes.length) {
      alert('No children nodes!');
    }
    childrenNodes[0].addEventListener('mouseenter', avatarsNodesHandler);
    childrenNodes[1].addEventListener('mouseenter', avatarsNodesHandler);
  }
})();
